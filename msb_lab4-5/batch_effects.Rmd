...
# Install BiocManager for bladderbatch
...
# https://stackoverflow.com/questions/20044744/how-to-produce-an-r-count-matrix
table_data <- table(pheno[, c("outcome", "batch")])
kable_data <- kable(table_data, caption="Distribution of tissues in different batches (columns).
All of the batches have one dominant tissue type in them.
This can lead to systematic differences that are not the
result of biological differences, but technical ones, like
variations in measurement conditions in each of the batches.")

# write.table(table_data, "park_problem1.txt", row.names=T, quote=T, sep=",")
write(kable_data, "park_problem1.txt")
...
batch_order <- rownames(pheno[order(pheno$batch), ]) # fetch sample (row) names after ordering batches
edata_reordered <- edata[, batch_order]
combat_edata_reordered <- combat_edata[, batch_order]

```

```{r}
heatmap.2(edata_reordered,
          main = "Bladder Cancer Data Clustered and Reordered by Batch", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
          ...
          scale="row",
          Rowv = T,   # cluster rows
          Colv = F)   # don't cluster columns

heatmap.2(combat_edata_reordered,
          main = "Bladder Cancer Data Cleaned by ComBat and Reordered by Batch", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
...
pdf("park_problem2.pdf", height=10, width=15)
heatmap.2(edata_reordered,
          main = "Bladder Cancer Data Clustered and Reordered by Batch", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
...
```{r}
rm(edata_reordered, combat_edata_reordered)
```
...
batch_sample_names <- paste(pheno$batch, pheno$outcome, rownames(pheno), sep = "-")
colnames(combat_edata) <- batch_sample_names

cor_matrix <- cor(combat_edata)

heatmap.2(cor_matrix,
          main = "Correlation matrix for samples", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
...
pdf("park_problem3.pdf",height=15,width=20)
heatmap.2(cor_matrix,
          main = "Correlation matrix for samples", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
...
>First, using a conventional approach, create a linear model with a genetic strain (biological variable) and an experimental number (technical variable) on **uncorrected** gene expression data. 
...
```{r}
# Now edata and pheno have bottomly data
load(file="bottomly.Rdata")
...
# 1. Linear model with a genetic strain and experimental number (on uncorrected data)
mod = lm(t(edata) ~ as.factor(pheno$strain) + as.factor(pheno$experiment.number))

# 2. Linear model with a genetic strain (on corrected data from ComBat)
...
ggplot(mod_tidy, aes(estimate, term)) +
     geom_point() + ggtitle("Coefficients for the linear model on uncorrected data") + theme(plot.title = element_text(hjust = 0.5))

ggplot(modcombat_tidy, aes(estimate, term)) +
     geom_point() + ggtitle("Coefficients for the linear model on corrected data") + theme(plot.title = element_text(hjust = 0.5))

# Histogram of p-values (as done in this notebook)

ggplot(mod_tidy %>% filter(term == "as.factor(pheno$strain)DBA/2J")) + geom_histogram(aes(x=p.value), bins = 100, fill="darkorange") + ggtitle("P-value histogram for linear model on uncorrected data") + theme(plot.title = element_text(hjust = 0.5))

ggplot(modcombat_tidy %>% filter(term == "as.factor(pheno$strain)DBA/2J")) + geom_histogram(aes(x=p.value), bins = 100, fill="darkorange") + ggtitle("P-value histogram for linear model on corrected data") + theme(plot.title = element_text(hjust = 0.5))



# Is this the scatter plot?
est_compare <- tibble(
...
```{r}
pdf("park_problem4.pdf",width=7,height=5)
...
head(sva_output$sv)
summary(lm(sva_output$sv ~ pheno$strain))
...
ggplot(modcombat_tidy, aes(estimate, term)) +
     geom_point() + ggtitle("Coefficients for the linear model on data corrected with ComBat") + theme(plot.title = element_text(hjust = 0.5))

ggplot(modsva_tidy, aes(estimate, term)) +
     geom_point() + ggtitle("Coefficients for the linear model on data corrected with SVA") + theme(plot.title = element_text(hjust = 0.5))


ggplot(pvalues_gather, aes(x=value)) + geom_histogram() + facet_wrap(~key) + ggtitle("Histogram of p-values comparing ComBat with SVA") + theme(plot.title = element_text(hjust = 0.5))
```


```{r}
pdf("park_problem5.pdf", width=7, height=5)
