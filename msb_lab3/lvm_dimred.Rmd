...
heatmap.2(edata,
          ...
          margins =c(12,9),
          col=my_palette,
          dendrogram="column",  # b) show a dendrogram only on the columns.
          scale = "column",     # c) scale in the column direction.
          Rowv = T,
          Colv = T              # a) both rows and columns are clustered,
          )     
dev.off()
...
udata <- as.data.frame(svd.out$u[,1:5])
data_for_plot <- gather(udata, key = "Vector", value = "Value")

names(udata)

pdf("park_problem4.pdf")
# ggplot(PC) + geom_violin(aes(x=as.factor(strain), y=V1),draw_quantiles = c(0.25, 0.5, 0.75)) + geom_jitter(aes(x=as.factor(strain), y=V1)) +
ggplot(data_for_plot, aes(x = Vector, y = Value, fill = Vector)) +
  geom_violin()
  # labs(x = "Left Singular Vectors", y = "Value") +
  # theme_minimal() +
...
k = 5
km <- stats::kmeans(edata, k)

pdf("park_problem5.pdf")
ggplot(tsne_out) + geom_point(aes(x=V1, y=V2, col=km$cluster), alpha=0.5) +
                    scale_color_gradientn(colours = rainbow(k))
dev.off()
```
